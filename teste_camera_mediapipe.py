...
from camera_pose import DetectorPose
...
class JogoAmarelinha:
    ...
    def __init__(self):
        ...
        self.tempo_entrada_elemento = {} # Armazena {id_elemento: tempo_inicio}
        self.tempo_delay = 2 # Tempo em segundos para confirmar o clique

        # --- Detector de Pose (Câmera) ---
        self.detector = DetectorPose(LARGURA_TELA, ALTURA_TELA)

        # --- Rastreamento em tempo real para debug ---
        self.pos_camera_atual = None  # Vai guardar (x, y) do ponto mapeado
    ...
    def processar_entrada_jogador(self, pos_mouse):
        ...
        if self.verificar_delay_interacao(f"casa_{casa_clicada}"):
            self._processar_jogada_casa(casa_clicada)

    def processar_interacao_menus(self, pos_virtual):
        """Processa a interação da câmera com os menus do jogo com debounce."""
        agora = time.time()
        # Cooldown global para evitar cliques acidentais entre telas diferentes
        if agora - getattr(self, 'ultimo_clique_menu', 0) < 1.0:
            return
        ...
        if self.estado == self.ESTADO_TELA_INICIAL:
            if self.botao_jogar.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_jogar"):
            ...
            if self.botao_modo_solo.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_modo_solo"):
                    self.modo_competitivo = False
                    self.estado = self.ESTADO_SELECAO
                    self.tempo_inicio_jogo = time.time()
                    clicou = True
            elif self.botao_modo_competitivo.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_modo_competitivo"):
                ...
                if self.verificar_delay_interacao("btn_voltar"):
                    self.estado = self.ESTADO_TELA_INICIAL
                    clicou = True
            ...
            for chave, rect in self.botoes_categoria.items():
                if rect.collidepoint(pos_virtual):
                    # Aqui usamos a chave da categoria como ID do elemento
                    if self.verificar_delay_interacao(f"cat_{chave}"):
                        self.selecionar_categoria(chave)
                        clicou = True
                        break
            if not clicou and self.botao_voltar.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_voltar_cat"):
                    if not (self.modo_competitivo and self.jogador_atual == 2):
                        self.estado = self.ESTADO_SELECAO_MODO
                        clicou = True

        elif self.estado == self.ESTADO_GAME_OVER:
            if hasattr(self, 'botao_jogar_novamente') and self.botao_jogar_novamente.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_novamente"):
                    cat = self.categoria_selecionada
                    self.__init__()
                    self.selecionar_categoria(cat)
                    clicou = True
            elif hasattr(self, 'botao_menu') and self.botao_menu.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_menu"):
                    self.__init__()
                    clicou = True

        elif self.estado == self.ESTADO_TRANSICAO_JOGADOR:
            if hasattr(self, 'botao_iniciar_j2') and self.botao_iniciar_j2.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_iniciar_j2"):
                    self._iniciar_jogador2()
                    clicou = True

        elif self.estado == self.ESTADO_CLASSIFICACAO:
            if hasattr(self, 'botao_classificacao_jogar') and self.botao_classificacao_jogar.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_class_jogar"):
                    ...
                    self.tempo_inicio_jogo = time.time()
                    clicou = True
            elif hasattr(self, 'botao_classificacao_menu') and self.botao_classificacao_menu.collidepoint(pos_virtual):
                if self.verificar_delay_interacao("btn_class_menu"):
                    self.__init__()
                    clicou = True

        if clicou:
            self.ultimo_clique_menu = agora

    def verificar_delay_interacao(self, elemento_id):
        agora = time.time()
        
        # Se o elemento mudou, reseta o timer
        if elemento_id not in self.tempo_entrada_elemento:
            self.tempo_entrada_elemento = {elemento_id: agora}
            return False
            
        # Verifica se já passou o tempo necessário (0.5s)
        if agora - self.tempo_entrada_elemento[elemento_id] >= self.tempo_delay:
            # Limpa o registro para permitir nova interação após sair/entrar
            del self.tempo_entrada_elemento[elemento_id]
            return True
    ...
    def verificar_dica(self):
        """Verifica se o jogador ficou ocioso."""
        # Se não estamos no modo de reproduzir, reseta o tempo da dica para 0
        if self.estado != self.ESTADO_REPRODUZIR:
            self.ultimo_clique_tempo = time.time()
            return

        # Se estamos no modo reproduzir, checa o tempo
    ...
    def desenhar(self):
        ...
        # --- DESENHO DO SEU CURSOR NA TELA DO JOGO ---
        if self.pos_camera_atual is not None:
            # Desenha a bolinha vermelha diretamente no canvas do jogo
        ...
        # Redimensiona a tela virtual para a janela real
    ...
    def executar(self):
        ...
        while rodando:
            # Captura a posição mapeada pela câmera/MediaPipe
            pos_pe = self.detector.detectar_posicao_pe()
            
            # Atualiza a variável global de desenho do cursor da câmera
            self.pos_camera_atual = pos_pe 

            if pos_pe is not None:
                # Mantém a lógica original do jogo para processar o clique/pisada
                self.processar_entrada_jogador(pos_pe)
                self.processar_interacao_menus(pos_pe)

            # Processamento de eventos padrão do Pygame
